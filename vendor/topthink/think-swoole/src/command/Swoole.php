...
class Swoole extends Command
{
    ...
    {
        //原代码 $pidFile = $this->config]['pid_file'];
        $pidFile = $this->config['option']['pid_file'];
        if (is_file($pidFile)) {
            $masterPid = (int) file_get_contents($pidFile);
        } else {
            $masterPid = 0;
        }
        SLog::info('SwooleConfig('.$pidFile.'|'.$masterPid.'):'.json_encode($this->config));
        return $masterPid;
    }
    ...
    protected function removePid()
    {
        ...
        if (is_file($masterPid)) {
            $unlinkResu = unlink($masterPid);
            SLog::info('removeImPid:'.$unlinkResu);
        }
    }
